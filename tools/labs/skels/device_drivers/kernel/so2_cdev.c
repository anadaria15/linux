...
static int so2_cdev_open(struct inode *inode, struct file *file)
{
	...
	if (atomic_cmpxchg(&(data->atomic_var), 0, 1) == 1)
		return -EBUSY;
...
static int so2_cdev_init(void)
{
	...
	pr_info("Device registration succeeded\n");
	...
	for (i = 0; i < NUM_MINORS; i++) {
		...
		atomic_set(&devs[i].atomic_var, 0);
...
static void so2_cdev_exit(void)
{
	...
	pr_info("Device unregistration succeeded\n");
}
