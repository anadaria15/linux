...
{
       	u8 scancode;
	int is_key_pressed, ascii;
	
	printk("SKB handler called\n");

	scancode = i8042_read_data();
	is_key_pressed = is_key_press(scancode);
	ascii = get_ascii(scancode);
	pr_info("IRQ %d: scancode=0x%x (%u) pressed=%d ch=%c\n",
        	irq_no, scancode, scancode, is_key_pressed, ascii);
                         ...
                         size_t count, loff_t * offset)
{
	...
	unsigned long local_buffer_size = 0;
        ...
        memset(local_buffer, 0, 300);
        if (copy_from_user(local_buffer, user_buffer, local_buffer_size))
                return -EFAULT;
			...
			size_t size, loff_t *offset)
{
	...
	while (size) {
		spin_lock_irqsave(&data->lock, flags);
		reading = get_char(&ch, data);
		spin_unlock_irqrestore(&data->lock, flags);
		if (!reading)
			break;
		ret = put_user(ch, user_buffer);
		if (ret)
			return -EFAULT;
		user_buffer ++;
		read ++;
